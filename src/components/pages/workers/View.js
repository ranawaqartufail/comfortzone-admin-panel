...
export default function ViewWorkers() {
  ...
  const [{ loading, data, error }, refetch] = useAuthedAxios("/worker/view");
    ...
    },
    {
      ...
      render: (_, record) => (
        <ButtonGroup>
          ...
          <Popconfirm
            title="Are you sure?"
            onConfirm={() => onDeleteConfirm(record._id, record.user.username)}
          >
  ...
  return (
    <MainLayout>
      ...
      />
      <Divider />
      <Spin spinning={loading} tip="Loading Workers...">
        {data && (
          <Table
            columns={columns}
